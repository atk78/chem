...
class Mol2Graph:
    ...
    def get_graph_features(self):
        ...
        if self.poly_flag:
            ...
            poly_bond_indexes = self.search_poly_bond(test_mol)  # 結合atom[*]のindexを探索
            test_mol = self.set_chain_type(test_mol, poly_bond_indexes)  # 構造中のatomが主鎖かどうか判別する
        else:
    ...
    def calc_graph_vector(
        ...
        if self.poly_flag:
            poly_bond_indexes = self.search_poly_bond(mol)  # 結合atom[*]のindexを探索
            mol = self.set_chain_type(mol, poly_bond_indexes)  # 構造中のatomが主鎖かどうか判別する
    ...
    @staticmethod
    def replace_poly_bond_to_atoms(mol, poly_bond_indexes):
        # 結合を隣の原子に置換
    ...
    @staticmethod
    def set_chain_type(mol, poly_bond_idx):

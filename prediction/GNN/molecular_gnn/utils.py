...
def compute_metrics(model, enum_graphs, card, scaler):
    model.eval()
    if len(enum_graphs) < 10000:
        batch_size = len(enum_graphs)
    else:
        batch_size = 10000
    dataloader = DataLoader(
        enum_graphs, batch_size=batch_size, shuffle=False, drop_last=False
    )
    y_pred_list = []
    y_list = []
    for dataset in dataloader:
        y = dataset.y.detach().numpy().copy().flatten()
        with torch.no_grad():

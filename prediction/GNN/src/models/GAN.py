...
class MolecularGAT(nn.Module):
    def __init__(self, n_features, n_conv_hidden=3, n_fc_hidden=3, embed_dim=128, dense_units=64, dropout=0.1):
        super().__init__()
        # graph layers
        self.gcn_hidden = nn.ModuleList()
        self.fc_hidden = nn.ModuleList()

        self.gcn1 = GATConv(n_features, n_features, heads=10, dropout=dropout)
        self.gcn2 = GATConv(n_features * 10, embed_dim, dropout=dropout)
        if n_conv_hidden - 1 > 0:
            for _ in range(n_conv_hidden - 1):
                self.gcn_hidden.append(GATConv(embed_dim, embed_dim, dropout=dropout))
        self.fc1 = nn.Linear(embed_dim, dense_units)
        if n_fc_hidden - 1 > 0:
            for _ in range(n_fc_hidden):
                self.mlp_hidden.append(nn.Linear(dense_units, dense_units))
...
class MolecularGAT(nn.Module):
    def __init__(
        ...
        self.gat_hidden = nn.ModuleList(
            [
                GATv2Conv(in_channels=self.dim, out_channels=self.dim)
                for _ in range(self.n_conv_hidden)
            ]
        )
        self.bn_conv_hidden = nn.ModuleList(
            [
                BatchNorm(in_channels=self.dim)
                for _ in range(self.n_conv_hidden)
            ]
        )
    ...
    def forward(self, data):
        ...
        x = F.leaky_relu(input=self.graphconv1(x, edge_index))
        x = self.bn1(x)
        for gat, bn_conv in zip(
            self.gat_hidden,
            self.bn_conv_hidden):
            x=gat(x, edge_index)
            x = bn_conv(x)
        # x = sag_pooling(x, edge_index)

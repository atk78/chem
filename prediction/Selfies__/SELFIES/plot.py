...
def plot_hitory_rmse(
    rmse, r2, save_dir=".", data_name=""
):
    ...
    fig.savefig(
        os.path.join(save_dir, f"images/History_fit_LSTMAtt_{data_name}.png"),
        bbox_inches="tight"
    )


def plot_obserbations_vs_predictions(
    observations, predictions, rmse, r2, save_dir=".", data_name=""
):
    y_train, y_valid, y_test = observations
    y_pred_train, y_pred_valid, y_pred_test = predictions
    rmse_train, rmse_valid, rmse_test = rmse
    ...
    ax.text(0.05, 0.95, f"Train RMSE: {rmse_train:.4f}", transform=ax.transAxes, fontsize=10)
    ax.text(0.05, 0.91, f"Train $R^2$: {r2_train:.4f}", transform=ax.transAxes, fontsize=10)
    ax.text(0.05, 0.87, f"Valid RMSE: {rmse_valid:.4f}", transform=ax.transAxes, fontsize=10)
    ax.text(0.05, 0.83, f"Valid $R^2$: {r2_valid:.4f}", transform=ax.transAxes, fontsize=10)
    ax.text(0.05, 0.79, f"Test RMSE: {rmse_test:.4f}", transform=ax.transAxes, fontsize=10)
    ax.text(0.05, 0.75, f"Test $R^2$: {r2_test:.4f}", transform=ax.transAxes, fontsize=10)
    fig.savefig(os.path.join(save_dir, f"images/Plot_LSTMAtt_{data_name}.png"))

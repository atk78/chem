...
def get_tokens(smiles_array: np.ndarray[str], split_l=1, poly_flag=False):
...
def extract_vocab(lltokens):
    return set([i_token for i_smiles in lltokens for i_token in i_smiles])
...
def int_vec_encode(
    ...
    for idx, i_smiles in enumerate(tokenized_smiles_list):
        ...
        if len(i_smiles) <= max_length:
            ...
            i_smiles_tmp = i_smiles_tmp[-max_length:]
        int_encoded = [

...
class SmilesVAE(nn.Module):
    ...
    def decode(self, z, out_seq=None, deterministic=False):
        ...
        if out_seq is None:
            with torch.no_grad():
                in_seq = torch.tensor(
                    [[self.tokenizer.sos_idx]] * batch_size, device=self.device
                )
                out_logit_list = []
    ...
    def generate(self, z=None, sample_size=None, deterministic=False):
        ...
        with torch.no_grad():
            ...
            out = [
                self.tokenizer.seq2smiles(each_seq, self.vocab)
    ...
    def reconstruct(
        ...
        for each_idx, each_seq in enumerate(in_seq):
            truth = self.tokenizer.seq2smiles(each_seq, self.vocab)[::-1]
            pred = self.tokenizer.seq2smiles(out_seq[each_idx], self.vocab)

...
class GMR(GaussianMixture):
    ...
    def predict(self, dataset, numbers_of_input_variables, numbers_of_output_variables):
        ...
        estimated_mean_for_all_components, _, weights = self.predict_mog(
            dataset, numbers_of_input_variables, numbers_of_output_variables
        )

        # calculate mode of estimated means and weighted estimated means
        mode_of_estimated_mean = np.empty(
            [dataset.shape[0], len(numbers_of_output_variables)]
        )
        weighted_estimated_mean = np.empty(
            [dataset.shape[0], len(numbers_of_output_variables)]
        )
        index_for_mode = np.argmax(weights, axis=0)
        for sample_number in range(dataset.shape[0]):
            mode_of_estimated_mean[sample_number, :] = (
                estimated_mean_for_all_components[
                    index_for_mode[sample_number], sample_number, :
                ]
            )
        ...
        return (
            ...
            weighted_estimated_mean,
            estimated_mean_for_all_components,
            weights,
        )

    def predict_rep(
        self, dataset, numbers_of_input_variables, numbers_of_output_variables
    ):
        """
        Gaussian Mixture Regression (GMR) based on Gaussian Mixture Model (GMM)
        ...
        mode_of_estimated_mean, weighted_estimated_mean, _, _ = self.predict(
            dataset, numbers_of_input_variables, numbers_of_output_variables
        )
        if self.rep == "mean":
    ...
    def predict_true(
        ...
        mode_of_estimated_mean_of_y, weighted_estimated_mean_of_y, _, _ = self.predict(
            dataset, numbers_of_input_variables, numbers_of_output_variables
        )
        estimated_means, estimated_covariances, weights = self.predict_mog(
            dataset, numbers_of_input_variables, numbers_of_output_variables
        )

        estimated_values_mode = self.true_gmr(
            estimated_means, estimated_covariances, weights, mode_of_estimated_mean_of_y
        )
        estimated_values_mean = self.true_gmr(
            estimated_means,
            estimated_covariances,
            weights,
            weighted_estimated_mean_of_y,
        )
        ...
        for i in range(dataset.shape[0]):
            ...
            tmp_func[0] = self.true_gmr_obj_func(
                mode_of_estimated_mean_of_y[i, :],
                estimated_means[:, i, :],
                estimated_covariances,
                weights[:, i],
            )
            tmp_estimated_values.append(mode_of_estimated_mean_of_y[i, :])
            tmp_func[1] = self.true_gmr_obj_func(
                weighted_estimated_mean_of_y[i, :],
                estimated_means[:, i, :],
                estimated_covariances,
                weights[:, i],
            )
            tmp_estimated_values.append(weighted_estimated_mean_of_y[i, :])
            tmp_func[2] = self.true_gmr_obj_func(
                estimated_values_mode[i, :],
                estimated_means[:, i, :],
                estimated_covariances,
                weights[:, i],
            )
            tmp_estimated_values.append(estimated_values_mode[i, :])
            tmp_func[3] = self.true_gmr_obj_func(
                estimated_values_mean[i, :],
                estimated_means[:, i, :],
                estimated_covariances,
                weights[:, i],
            )
    ...
    def true_gmr_obj_func(self, variable, means, covariances, weights):
        ...
        for i in range(covariances.shape[0]):
            tmp = np.log(weights[i]) + multivariate_normal.logpdf(
                variable, mean=means[i, :], cov=covariances[i, :, :]
            )
            tmps.append(tmp)
    ...
    def true_gmr(self, means_all, covariances, weights_all, init_values, bounds=[]):
        ...
        for i in range(number_of_samples):
            ...
            if len(bounds) == 0:
                pred_results = minimize(
                    self.true_gmr_obj_func,
                    x0=init_value,
    ...
    def predict_logpdf(
        ...
        estimated_means_all, estimated_covariances, weights_all = self.predict_mog(
            dataset, numbers_of_input_variables, numbers_of_output_variables
        )
        ...
        if estimated_results.ndim == 0:
            ...
            estimated_results = np.reshape(
                estimated_results, (1, estimated_results.shape[0])
            )
        ...
        for sample_number in range(estimated_results.shape[0]):
            ...
            for component_number in range(estimated_covariances.shape[0]):
                tmp = np.log(weights[component_number]) + multivariate_normal.logpdf(
                    estimated_results[sample_number, :],
                    mean=estimated_means[component_number, :],
                    cov=estimated_covariances[component_number, :, :],
                )
                tmps.append(tmp)
    ...
    def predict_mog(
        ...
        estimated_means = np.zeros(
            [self.n_components, dataset.shape[0], len(numbers_of_output_variables)]
        )
        estimated_covariances = np.zeros(
            [
                self.n_components,
                len(numbers_of_output_variables),
                len(numbers_of_output_variables),
            ]
        )
        ...
        if all_covariances.shape[2] == len(numbers_of_input_variables) + len(
            numbers_of_output_variables
        ):
            input_output_covariances = all_covariances[:, numbers_of_input_variables, :]
            input_covariances = input_output_covariances[
                :, :, numbers_of_input_variables
            ]
            input_output_covariances = input_output_covariances[
                :, :, numbers_of_output_variables
            ]
            output_input_covariances = all_covariances[
                :, numbers_of_output_variables, :
            ]
            output_covariances = output_input_covariances[
                :, :, numbers_of_output_variables
            ]
            output_input_covariances = output_input_covariances[
                :, :, numbers_of_input_variables
            ]

            # estimated means and weights for all components
            for component_number in range(self.n_components):
                estimated_means[component_number, :, :] = output_means[
                    component_number, :
                ...
                estimated_covariances[component_number, :, :] = output_covariances[
                    component_number, :, :
                ...
                try:
                    weights[component_number, :] = self.weights_[
                        component_number
    ...
    def cv_opt(
        ...
        index = np.matlib.repmat(
            np.arange(1, fold_number + 1, 1), 1, min_number
        ).ravel()
        if mod_number != 0:
        ...
        for covariance_type in covariance_types:
            for number_of_components in numbers_of_components:
                for rep in reps:
                    ...
                    estimated_y_in_cv = np.zeros(
                        [dataset.shape[0], len(numbers_of_output_variables)]
                    )

                    for fold_number_in_cv in np.arange(1, fold_number + 1, 1):
                        dataset_train_in_cv = dataset[
                            fold_index_in_cv != fold_number_in_cv, :
                        ]
                        dataset_test_in_cv = dataset[
                            fold_index_in_cv == fold_number_in_cv, :
                        ]
                        try:
                            self.fit(dataset_train_in_cv)
                            values = self.predict_rep(
                                dataset_test_in_cv[:, numbers_of_input_variables],
                                numbers_of_input_variables,
                                numbers_of_output_variables,
                            )
                        except:
                            values = np.ones(
                                [
                                    dataset_test_in_cv.shape[0],
                                    len(numbers_of_output_variables),
                                ]
                            ) * (-(10**10))

                        estimated_y_in_cv[fold_index_in_cv == fold_number_in_cv, :] = (
                            values  # 格納
                        )
    ...
    def cv_bo(
        ...
        # GMRのパラメータ設定
        gmr_kfold = KFold(
            n_splits=fold_number, shuffle=True, random_state=2
        )  # CVの設定
        ...
        params_df = pd.DataFrame(
            params_temp_list, columns=["number of components", "covariance type", "rep"]
        )  # 全組み合わせのDataframe
        params_df = pd.get_dummies(
            params_df, columns=["covariance type", "rep"]
        )  # ダミー変数化
        ...
        if params_df.shape[0] <= doe_number_of_selecting_samples + bo_iteration_number:
            ...
            for bo_iter in bo_iterations:
                if self.display_flag:
                    print(
                        f"Bayesian optimization iteration : {bo_iter + 1} / {bo_iteration_number}"
                    )
                #    print('='*10)
                if bo_iter == 0:  # 最初の試行ではD最適基準を計算
                    # D最適基準の計算
                    autoscaled_params_df = (
                        params_df - params_df.mean(axis=0)
                    ) / params_df.std(
                        axis=0, ddof=1
                    )  # 計算のために標準化

                    all_indexes = list(
                        range(autoscaled_params_df.shape[0])
                    )  # indexを取得
                    ...
                    for random_search_number in range(doe_number_of_random_searches):
                        ...
                        new_selected_indexes = np.random.choice(
                            all_indexes, doe_number_of_selecting_samples, replace=False
                        )
                        new_selected_samples = autoscaled_params_df.iloc[
                        ...
                        if random_search_number == 0:
                            ...
                            if best_d_optimal_value < d_optimal_value:
                                best_d_optimal_value = d_optimal_value.copy()
                                selected_sample_indexes = new_selected_indexes.copy()
                    selected_sample_indexes = list(
                        selected_sample_indexes
                    )  # リスト型に変換

                    # 選択されたサンプル、選択されなかったサンプル
                    selected_params_df = params_df.iloc[
                        selected_sample_indexes, :
                    ]  # 選択されたサンプル
                    bo_params_df = (
                        selected_params_df.copy()
                    )  # BOのGPモデル構築用データを作成
                    remaining_indexes = np.delete(
                        all_indexes, selected_sample_indexes
                    )  # 選択されなかったサンプルのインデックス
                    remaining_params_df = params_df.iloc[
                        remaining_indexes, :
                    ]  # 選択されなかったサンプル

                    # 選択された全候補でGMRの計算
                    params_with_score_df = (
                        params_df.copy()
                    )  # cvのscoreが含まれるdataframe
                    params_with_score_df["r2cv score"] = np.nan  # 初期値はnanを設定

                else:  # 2回目以降では前回の結果をもとにする
                    selected_sample_indexes = (
                        next_samples_df.index
                    )  # 提案サンプルのindex
                    selected_params_df = params_df.loc[
                        selected_sample_indexes, :
                    ]  # 次に計算するサンプル
                    bo_params_df = pd.concat(
                        [bo_params_df, selected_params_df], axis=0
                    )  # BOのGPモデル構築用データは前回のデータと提案サンプルをマージする
                    remaining_params_df = params_df.loc[
                        params_with_score_df["r2cv score"].isna(), :
                    ]  # 選択されなかったサンプル
                    remaining_params_df = remaining_params_df.drop(
                        index=selected_sample_indexes
                    )

                # 選ばれたサンプル（パラメータの組み合わせ）を一つずつ計算する
                for i_n, selected_params_idx in enumerate(selected_sample_indexes):
                    selected_params = selected_params_df.loc[
                        selected_params_idx, :
                    ]  # サンプルの選択
                    # componentの数の決定
                    number_of_components = selected_params["number of components"]
                    # covariance typeの決定
                    covariance_series = selected_params[
                        [x for x in selected_params.index if "covariance type" in x]
                    ]
                    covariance_type = covariance_series.loc[covariance_series == 1]
                    covariance_type = (covariance_type.index[0]).replace(
                        "covariance type_", ""
                    )
                    # repの決定
                    rep_series = selected_params[
                        [x for x in selected_params.index if "rep" in x]
                    ]
                    ...
                    self.n_components = number_of_components
                    self.rep = rep_type
                    estimated_y_in_cv = np.empty(
                        (dataset.shape[0], len(numbers_of_output_variables))
                    )  # yの保存先
                    for i, (cv_train_idx, cv_test_idx) in enumerate(
                        gmr_kfold.split(dataset)
                    ):  # CVによる検証
                        # CVのinnerとouterを設定
                        ...
                        try:
                            ...
                            predict_y_train_outercv = self.predict_rep(
                                autoscaled_train_outercv[:, numbers_of_input_variables],
                                numbers_of_input_variables,
                                numbers_of_output_variables,
                            )
                        except:
                            predict_y_train_outercv = np.ones(
                                [autoscaled_train_outercv.shape[0], 1]
                            ) * (-(10**10))
                    ...
                    params_with_score_df.loc[selected_params_idx, "r2cv score"] = (
                        gmr_r2_score  # データの保存
                    )
                if self.display_flag:
                ...
                bo_x_data = (
                    bo_params_df.copy()
                )  # GP学習用データはGMRの結果があるサンプル
                bo_x_prediction = (
                    remaining_params_df.copy()
                )  # predictionは選択されていない（GMRの結果がない）サンプル
                bo_y_data = params_with_score_df.loc[
                    bo_params_df.index, "r2cv score"
                ]  # yはGMRのr2cv
                ...
                bo_kernels = [
                    ...
                    ConstantKernel() * RBF(np.ones(bo_x_data.shape[1]))
                    + WhiteKernel()
                    + ConstantKernel() * DotProduct(),
                ...
                next_samples = pd.DataFrame(
                    [], columns=selected_params_df.columns
                )  # 次のサンプルを入れる変数を準備
                ...
                for bo_sample_number in range(bo_number_of_selecting_samples):
                    ...
                    autoscaled_bo_y_data = (
                        bo_y_data - bo_y_data.mean()
                    ) / bo_y_data.std()
                    autoscaled_bo_x_data = (
                        bo_x_data - bo_x_data.mean()
                    ) / bo_x_data_std
                    autoscaled_bo_x_prediction = (
                        bo_x_prediction - bo_x_data.mean()
                    ) / bo_x_data_std
                    ...
                    if bo_regression_method == "gpr_one_kernel":
                        ...
                        bo_model = GaussianProcessRegressor(
                            alpha=0, kernel=bo_selected_kernel
                        )

                    elif bo_regression_method == "gpr_kernels":
                        # クロスバリデーションによるカーネル関数の最適化
                        bo_cross_validation = KFold(
                            n_splits=bo_gp_fold_number, random_state=9, shuffle=True
                        )  # クロスバリデーションの分割の設定
                        bo_r2cvs = (
                            []
                        )  # 空の list。カーネル関数ごとに、クロスバリデーション後の r2 を入れていきます
                        for index, bo_kernel in enumerate(bo_kernels):
                            bo_model = GaussianProcessRegressor(
                                alpha=0, kernel=bo_kernel
                            )
                            estimated_bo_y_in_cv = np.ndarray.flatten(
                                cross_val_predict(
                                    bo_model,
                                    autoscaled_bo_x_data,
                                    autoscaled_bo_y_data,
                                    cv=bo_cross_validation,
                                )
                            )
                            estimated_bo_y_in_cv = (
                                estimated_bo_y_in_cv * bo_y_data.std(ddof=1)
                                + bo_y_data.mean()
                            )
                            bo_r2cvs.append(r2_score(bo_y_data, estimated_bo_y_in_cv))
                        optimal_bo_kernel_number = np.where(
                            bo_r2cvs == np.max(bo_r2cvs)
                        )[0][
                            0
                        ]  # クロスバリデーション後の r2 が最も大きいカーネル関数の番号
                        optimal_bo_kernel = bo_kernels[
                            optimal_bo_kernel_number
                        ]  # クロスバリデーション後の r2 が最も大きいカーネル関数

                        # モデル構築
                        bo_model = GaussianProcessRegressor(
                            alpha=0, kernel=optimal_bo_kernel
                        )  # GPR モデルの宣言

                    bo_model.fit(
                        autoscaled_bo_x_data, autoscaled_bo_y_data
                    )  # モデルの学習

                    # 予測
                    estimated_bo_y_prediction, estimated_bo_y_prediction_std = (
                        bo_model.predict(autoscaled_bo_x_prediction, return_std=True)
                    )
                    estimated_bo_y_prediction = (
                        estimated_bo_y_prediction * bo_y_data.std() + bo_y_data.mean()
                    )
                    estimated_bo_y_prediction_std = (
                        estimated_bo_y_prediction_std * bo_y_data.std()
                    )
                    ...
                    if acquisition_function == "MI":
                        acquisition_function_prediction = (
                            ...
                            + np.log(2 / delta) ** 0.5
                            * (
                                (estimated_bo_y_prediction_std**2 + cumulative_variance)
                                ** 0.5
                                - cumulative_variance**0.5
                            )
                        )
                        cumulative_variance = (
                            cumulative_variance + estimated_bo_y_prediction_std**2
                        )
                    elif acquisition_function == "EI":
                        acquisition_function_prediction = (
                            estimated_bo_y_prediction
                            - max(bo_y_data)
                            - relaxation * bo_y_data.std()
                        ) * norm.cdf(
                            (
                                estimated_bo_y_prediction
                                - max(bo_y_data)
                            ...
                            / estimated_bo_y_prediction_std
                        ) + estimated_bo_y_prediction_std * norm.pdf(
                            (
                                estimated_bo_y_prediction
                                - max(bo_y_data)
                                - relaxation * bo_y_data.std()
                    ...
                    acquisition_function_prediction[
                        estimated_bo_y_prediction_std <= 0
                    ] = 0

                    # 保存
                    estimated_bo_y_prediction = pd.DataFrame(
                        estimated_bo_y_prediction,
                        bo_x_prediction.index,
                        columns=["estimated_y"],
                    )
                    estimated_bo_y_prediction_std = pd.DataFrame(
                        estimated_bo_y_prediction_std,
                        bo_x_prediction.index,
                        columns=["std_of_estimated_y"],
                    )
                    acquisition_function_prediction = pd.DataFrame(
                        acquisition_function_prediction,
                        index=bo_x_prediction.index,
                        columns=["acquisition_function"],
                    )
                    #
                    # 次のサンプル
                    next_samples = pd.concat(
                        [
                            next_samples,
                            bo_x_prediction.loc[
                                acquisition_function_prediction.idxmax()
                            ],
                        ],
                        axis=0,
                    )
                    ...
                    bo_y_data = pd.concat(
                        [
                            bo_y_data,
                            estimated_bo_y_prediction.loc[
                                acquisition_function_prediction.idxmax()
                            ].iloc[0],
                        ],
                        axis=0,
                    )
                    bo_x_prediction = bo_x_prediction.drop(
                        acquisition_function_prediction.idxmax(), axis=0
                    )
                    cumulative_variance = np.delete(
                        cumulative_variance,
                        np.where(
                            acquisition_function_prediction.index
                            == acquisition_function_prediction.iloc[:, 0].idxmax()
                        )[0][0],
                    )
            ...
            params_with_score_df_best = params_with_score_df.sort_values(
                "r2cv score", ascending=False
            ).iloc[
                0, :
            ]  # r2が高い順にソー
            # covariance typeの決定
            covariance_series = params_with_score_df_best[
                [x for x in params_with_score_df_best.index if "covariance type" in x]
            ]
            covariance_type = covariance_series.loc[covariance_series == 1]
            # repの決定
            rep_series = params_with_score_df_best[
                [x for x in params_with_score_df_best.index if "rep" in x]
            ]
            rep_type = rep_series.loc[rep_series == 1]
            self.covariance_type = (covariance_type.index[0]).replace(
                "covariance type_", ""
            )

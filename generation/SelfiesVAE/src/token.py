...
class SmilesVocabulary:
    ...
    def update(self, smiles):
        ...
        torch.Tensor
            SMILESに対応する整数系列
        """
        selfies = re.findall(pattern=r"\[[^\[\]]*]", string=sf.encoder(smiles))
        char_set = set(selfies)
    ...
    def smiles2seq(self, smiles):
        ...
        return torch.tensor(
            [self.sos_idx]
            + [self.char_list.index(each_char) for each_char in selfies]
            + [self.eos_idx]
        )
    ...
    def seq2smiles(self, seq, wo_special_char=True):
        ...
        # すべて結合してSMILESに変換
        selfies = "".join([self.char_list[each_idx] for each_idx in seq])

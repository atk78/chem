...
def parse_amino_acids(a1):
    ...
    new_a = Chem.ReplaceSubstructs(
        a1, amino, Chem.MolFromSmiles("N[*:1]"), useChirality=True
    )[0]
    new_a = Chem.ReplaceSubstructs(
        new_a,
        carboxylic,
        Chem.MolFromSmiles("C(=O)[*:1]"),
...
def plot_length_histogram(
    string_sets, alpha=0.5, boxes=10, names=None
):  # can be list of tokens, not necessarily strings
    if not names:
    ...
    total_lens = list(
        itertools.chain.from_iterable(
            [
                [(x if type(x) is int else len(x)) for x in string_set]
    ...
    for name, string_set in zip(names, string_sets):
        lens = [(x if type(x) is int else len(x)) for x in string_set]
        ...
        plt.bar(
            list(range(m_lens, M_lens + 1, box_size)),
            counts,
            width=box_size,
            alpha=alpha,
            label=name,
